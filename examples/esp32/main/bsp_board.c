...
//     if (ret != ESP_OK) {
//         ESP_LOGE(TAG, "Failed to create new I2C bus: %s", esp_err_to_name(ret));
//         return ret;
//     }

//     ESP_LOGI(TAG, "I2C%d initialized (driver NG) with speed %d", i2c_num, clk_speed);
//     return ESP_OK;
// }
esp_codec_dev_handle_t bsp_get_record_dev(void)
{
    return record_dev;
}
esp_err_t bsp_i2c_init(i2c_port_t i2c_num, uint32_t clk_speed)
{
    ...
    ESP_LOGI(TAG, "I2C%d initialized with speed %d", i2c_num, clk_speed);
    return i2c_driver_install(i2c_num, i2c_cfg.mode, 0, 0, 0);
}

esp_err_t bsp_codec_adc_init(int sample_rate)
{
    ESP_LOGI(TAG, "bsp_codec_adc_init: sample_rate=%d", sample_rate);
    ...
    ESP_LOGI(TAG, "record_dev=%p, record_data_if=%p, record_codec_if=%p", record_dev, record_data_if, record_codec_if);
    if (record_dev == NULL) {
        ESP_LOGE(TAG, "Failed to initialize record device!");
        return ESP_FAIL;
    }else {
        ESP_LOGI(TAG, " Succesfully initialize record device!");
    }
...
ESP_LOGI(TAG, "bsp_codec_adc_init: record_dev=%d",ret_val );
    return ret_val;
}

esp_err_t bsp_codec_dac_init(int sample_rate, int channel_format, int bits_per_chan)
{
    ESP_LOGI(TAG, "bsp_codec_dac_init: sample_rate=%d, channel_format=%d, bits_per_chan=%d", sample_rate, channel_format, bits_per_chan);
    esp_err_t ret_val = ESP_OK;
...
ESP_LOGI(TAG, "bsp_codec_dac_init: play_dev=%d", ret_val);
    return ret_val;
}
...
{
    ESP_LOGI(TAG, "bsp_i2s_init: i2s_num=%d, sample_rate=%d, channel_format=%d, bits_per_chan=%d", i2s_num, sample_rate, channel_format, bits_per_chan);
    esp_err_t ret_val = ESP_OK;
...
#endif
ESP_LOGI(TAG, "bsp_i2s_init: ret_val=%d", ret_val);
    return ret_val;
}
...
{
    ESP_LOGI(TAG, "bsp_audio_play: length=%d, ticks_to_wait=%d", length, ticks_to_wait);
    size_t bytes_write = 0;
    esp_err_t ret = ESP_OK;
    if (!play_dev) {
        return ESP_FAIL;
    }
ESP_LOGI(TAG, "bsp_audio_play: play_dev=%d", play_dev);
...
{
    ESP_LOGI(TAG, "bsp_board_init: sample_rate=%d, channel_format=%d, bits_per_chan=%d", sample_rate, channel_format, bits_per_chan);
...
ESP_LOGI(TAG, "I2s in starting phase");
    bsp_i2s_init(I2S_NUM_1, 16000, 2, 32);
    ESP_LOGI(TAG, "I2S bus initialized");
    // Because record and play use the same i2s.
    ESP_LOGI(TAG, "bsp_codec_init: sample_rate=%d, channel_format=%d, bits_per_chan=%d", 16000, 2, 32);
