...
//     g711_cfg.sample_rate = 16000;
//     g711_cfg.channel = 2;
//     g711_cfg.bits_per_sample =16;
//     g711_cfg.frame_duration = 5;
...
void audio_task(void* arg) {
    ...
    for (;;) {
        if (eState == PEER_CONNECTION_COMPLETED) {
            curr_time = get_timestamp();
             ESP_LOGI(TAG, "Audio task started 1 ");
            ...
            // }
            ESP_LOGI(TAG, "Audio task started 2");
            taskYIELD();
           // printf("inside the audio task:%\n",aenc_in_frame.buffer);
           // printf("inside the audio task:%ld\n",aenc_in_frame.len );
            ret = audio_get_samples(aenc_in_frame.buffer, aenc_in_frame.len);
            if (ret == aenc_in_frame.len) {
                taskYIELD();
                 ESP_LOGI(TAG, "Audio task started 3");
                esp_audio_err_t enc_ret = esp_audio_enc_process(enc_handle, &aenc_in_frame, &aenc_out_frame);   
                if (enc_ret == ESP_AUDIO_ERR_OK) {
                    int send_ret = peer_connection_send_audio(g_pc, aenc_out_frame.buffer, aenc_out_frame.encoded_bytes);
                    if (send_ret >= 0) {
                 ...
                 ESP_LOGI(TAG, "Audio task started 4");
                // ESP_LOGI(TAG, "g_pc: %p | buffer: %p | encoded_bytes: %d", g_pc, aenc_out_frame.buffer, aenc_out_frame.encoded_bytes);
                // if (g_pc && aenc_out_frame.buffer && aenc_out_frame.encoded_bytes > 0) {
                //     int send_ret = peer_connection_send_audio(g_pc, aenc_out_frame.buffer, aenc_out_frame.encoded_bytes);
                //     if (send_ret < 0) {
                //         ESP_LOGW(TAG, "Send audio failed: %d", send_ret);
                //     }
                // }
            }
